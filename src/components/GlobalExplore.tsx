...
export function GlobalExplore(){
 ...
 return <div className={`${styles.page} ${styles.exploreV2}`}>
  <div className={styles.exploreHero}><div><span>EXPLORE O VINDE</span><h1>Um lugar para viver sua fé por inteiro.</h1><p>Descubra oração, comunidade, viagens, eventos e escolhas com propósito.</p></div></div>
  ...
  <div className={styles.explorePaths}>{paths.map(({href,label,text,icon:Icon})=><Link href={href} key={href}><span><Icon size={20}/></span><div><strong>{label}</strong><small>{text}</small></div></Link>)}</div>
  ...
  {loading?<div className={styles.empty}>Carregando experiências…</div>:featured?<><Link href={featured.href} className={styles.exploreFeatured} style={{backgroundImage:`linear-gradient(0deg,rgba(16,35,58,.84),rgba(16,35,58,.04)),url(${featured.image})`}}><span>{labels[featured.kind]}</span><h2>{featured.title}</h2><p>{featured.subtitle}</p><div>Explorar <ChevronRight size={16}/></div></Link>{rest.length>0&&<div className={styles.exploreFeed}>{rest.map(x=><Link href={x.href} key={`${x.kind}-${x.id}`}><img src={x.image} alt=""/><div><small>{labels[x.kind]}</small><strong>{x.title}</strong><p>{x.subtitle}</p></div><ChevronRight size={17}/></Link>)}</div>}<Link href="/momento" className={styles.exploreClosing}><Sparkles size={19}/><div><small>UM MINUTO PARA VOCÊ</small><strong>Comece pela Palavra de hoje.</strong></div><ChevronRight size={17}/></Link></>:<div className={styles.empty}>Nada encontrado para esta busca.</div>}
 </div>
}
