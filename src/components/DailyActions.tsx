...
import {Heart,Share2,Check,Flame,BarChart3} from "lucide-react";
...
export function DailyActions({devotionalId,title}:{devotionalId?:string;title:string}){
 const {user,profile}=useAuth();const [done,setDone]=useState(false);const [fav,setFav]=useState(false);const [dates,setDates]=useState<string[]>([]);const [busy,setBusy]=useState(false);
 useEffect(()=>{if(!user||!devotionalId)return;const userId=user.id;const s=getSupabase();Promise.all([s.from("devotional_completions").select("devotional_id,completed_at,daily_devotionals(devotional_date)").eq("user_id",userId).order("completed_at",{ascending:false}).limit(60),s.from("favorites").select("entity_id").eq("user_id",userId).eq("entity_type","devotional").eq("entity_id",devotionalId).maybeSingle()]).then(([a,b])=>{const rows=(a.data||[]) as any[];setDone(rows.some(x=>x.devotional_id===devotionalId));setDates(rows.map(x=>x.daily_devotionals?.devotional_date).filter(Boolean));setFav(!!b.data)});track("daily_moment_open",{entity_type:"devotional",entity_id:devotionalId,parish_id:profile?.primary_parish_id||undefined,path:"/momento"})},[user?.id,devotionalId,profile?.primary_parish_id]);
