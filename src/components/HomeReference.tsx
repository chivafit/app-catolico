...
export function HomeReference({devotional,event,recommendations=[]}:{devotional:Devotional|null;event:Event|null;recommendations?:FaithRecommendation[]}){
 ...
 return <div className={styles.page}>
  <header className={styles.header}><div className={styles.brand}><span className={styles.brandMark}>✣</span><div className={styles.brandText}><strong>Vinde</strong><small>Mais perto de Deus,<br/>mais perto das pessoas.</small></div></div><Link href={user?"/perfil":"/entrar"} className={styles.avatar}>{initials}</Link></header>
  <Link href="/momento" className={styles.hero} style={{backgroundImage:`url(${banner})`}}><span className={styles.heroKicker}>SEU MOMENTO · 5 MIN</span><h1>{devotional?.title||"Descansar em Deus"}</h1><p>{devotional?.scripture_reference||"Mt 11,28–30"}</p><div className={styles.heroCta}>Começar agora <ArrowRight size={17}/></div></Link>
  ...
  <section className={styles.section}><div className={styles.sectionHead}><h2>Hoje no Vinde</h2><Link href="/explorar">Ver tudo</Link></div><Link href="/igreja" className={styles.feature}><span className={styles.featureIcon}><Church/></span><div><strong>Minha Igreja</strong><small>Comunidade, horários e vida paroquial</small></div><ArrowRight size={18}/></Link></section>
  {event&&<section className={styles.section}><div className={styles.sectionHead}><h2>Na sua comunidade</h2><Link href="/eventos">Agenda</Link></div><Link href="/eventos" className={styles.event}><img src={event.image_url||eventFallback} alt=""/><div><span>PRÓXIMO EVENTO</span><h3>{event.title}</h3><p><MapPin size={11}/> {event.parishes?.name||event.location||"Sua paróquia"}</p>{evDate&&<small>{evDate.toLocaleDateString("pt-BR",{day:"2-digit",month:"long"})} · {evDate.toLocaleTimeString("pt-BR",{hour:"2-digit",minute:"2-digit"})}</small>}</div><ArrowRight size={17}/></Link></section>}
  {recommendations.length>0&&<section className={styles.section}><div className={styles.sectionHead}><h2><Sparkles size={18}/> Para você</h2><Link href="/explorar">Explorar</Link></div><div className={styles.recs}>{recommendations.slice(0,3).map(r=><Link href={r.href} key={r.id} className={styles.rec}><span className={styles.recIcon}>{recIcon(r.kind)}</span><div><strong>{r.title}</strong><small>{r.subtitle.replace(/\s+[—-]\s+Demo\b/gi,"")}</small></div><ArrowRight size={17}/></Link>)}</div></section>}
 </div>
}
