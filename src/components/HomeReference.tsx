...
import {BookOpen,Heart,Bus,CalendarDays,ShoppingBag,Church,Sparkles,ArrowRight,MapPin,Clock3} from "lucide-react";
...
export function HomeReference({devotional,event,parish,massSchedules=[],recommendations=[]}:{devotional:Devotional|null;event:Event|null;parish?:Parish|null;massSchedules?:MassSchedule[];recommendations?:FaithRecommendation[]}){
 ...
 const now=new Date();const hour=now.getHours();const greeting=hour<12?"Bom dia":hour<18?"Boa tarde":"Boa noite";const dateLabel=now.toLocaleDateString("pt-BR",{weekday:"long",day:"numeric",month:"long"});
 ...
 return <div className="home-v3">
  <header className="home-v3-head"><div><span className="home-v3-date">{dateLabel}</span><h1>{greeting}, {first}</h1></div><div className="home-v3-head-actions"><span className="streak"><Sparkles size={15}/> Sua caminhada</span><Link href={user?"/perfil":"/entrar"} className="home-v3-avatar">{initials}</Link></div></header>
  ...
  <section className="week-v3"><div><strong>Sua caminhada</strong><small>Um registro de presença, não uma competição.</small></div><div className="week-days">{["S","T","Q","Q","S","S","D"].map((d,i)=><span className={i<3?"done":i===3?"today":""} key={i}>{i<3?"✓":d}</span>)}</div></section>
