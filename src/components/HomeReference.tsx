...
import {BookOpen,Heart,ShoppingBag,Church,UsersRound,ArrowRight,ChevronRight,MoreHorizontal,MapPin,HandHeart,CalendarHeart,Sparkles} from "lucide-react";
...
export function HomeReference({devotional,event,recommendations=[]}:{devotional:Devotional|null;event:Event|null;recommendations?:FaithRecommendation[]}){
 ...
 return <div className={styles.page}>
  ...
  <section className={styles.quickWrap}><span className={styles.quickLabel}>Acesso rápido</span><div className={styles.quick}>{quick.map(({href,label,icon:Icon})=><Link href={href} key={href}><Icon size={22}/><span>{label}</span></Link>)}</div></section>
  ...
  <section className={styles.section}><div className={styles.sectionHead}><h2><Sparkles size={18}/> Para o seu caminho</h2><Link href="/explorar">Ver mais</Link></div><div className={styles.journeyCard}><Link href="/momento" className={styles.journeyFeatured}><div className={styles.journeyThumb} style={{backgroundImage:`url(${banner})`}}><span>PALAVRA DE HOJE</span></div><div className={styles.journeyFeaturedCopy}><div><strong>Momento Diário</strong><small>{devotional?.scripture_reference||devotional?.title||"Evangelho, reflexão e oração para hoje"}</small></div><ChevronRight size={18}/></div></Link>{secondaryRecommendations.map(r=><Link href={r.href} key={r.id} className={styles.journeyRow}><span className={styles.recIcon}>{recIcon(r.kind)}</span><div><strong>{r.title}</strong><small>{r.subtitle.replace(/\s+[—-]\s+Demo\b/gi,"")}</small></div><ChevronRight size={17}/></Link>)}</div></section>
 </div>
}
