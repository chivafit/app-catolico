...
import {Apple,Shirt,Gift,TreePine,Backpack,HeartHandshake,MapPin,ChevronDown,ShieldCheck,CalendarDays,Heart,ArrowRight,Check} from "lucide-react";
...
export function ParishCampaigns(){
 ...
 return <AppShell><div className={styles.page}>
  ...
  <section className={styles.section}><div className={styles.sectionHead}><div><small>CAMPANHAS DA PARÓQUIA</small><h2>Campanhas disponíveis</h2></div><span>{baseCampaigns.length}</span></div><p className={styles.sectionIntro}>Estas campanhas fazem parte da base do Vinde e ficam sempre visíveis para cada paróquia. A comunidade define quando cada ação está recebendo itens e onde será a entrega.</p><div className={styles.templateGrid}>{baseCampaigns.map(({key,title,subtitle,icon:Icon,tag,description,items})=><article className={styles.templateCard} key={key}><div className={styles.templateTop}><span className={styles.templateIcon}><Icon size={21}/></span><div><small>{tag}</small><strong>{title}</strong><p>{subtitle}</p></div></div><div className={styles.templateDetail}><p>{description}</p><strong>O que doar</strong><div className={styles.itemList}>{items.map(item=><span key={item}><Check size={12}/>{item}</span>)}</div><div className={styles.templateFooter}><span><Heart size={14}/>Entrega e datas são definidas pela paróquia.</span></div></div></article>)}</div></section>
