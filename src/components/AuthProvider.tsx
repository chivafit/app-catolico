...
export function AuthProvider({children}:{children:ReactNode}){
 ...
 async function loadProfile(nextUser:User|null){
  setUser(nextUser);if(!nextUser){setProfile(null);setLoading(false);return}
  const s=getSupabase();
  await s.from("profiles").upsert({id:nextUser.id},{onConflict:"id",ignoreDuplicates:true});
  const {data}=await s.from("profiles").select("*").eq("id",nextUser.id).maybeSingle();setProfile((data as Profile)||null);setLoading(false);
 }
 async function refreshProfile(){if(user) await loadProfile(user)}
 async function signOut(){await getSupabase().auth.signOut();setUser(null);setProfile(null)}
 useEffect(()=>{const s=getSupabase();s.auth.getUser().then(({data})=>loadProfile(data.user));const {data:listener}=s.auth.onAuthStateChange((_event,session)=>{loadProfile(session?.user||null)});return()=>listener.subscription.unsubscribe()},[]);
