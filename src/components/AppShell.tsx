...
const titles:Record<string,string>={momento:"Momento Diário",peregrinacoes:"Peregrinações",oracoes:"Orações",livros:"Livraria",loja:"Loja",eventos:"Eventos",campanhas:"Campanhas",carrinho:"Carrinho",configuracoes:"Configurações",favoritos:"Favoritos",paroquias:"Paróquias",termos:"Termos de Uso",privacidade:"Privacidade"};
...
export function AppShell({children,hideHeader=false,hideNav=false}:{children:ReactNode;hideHeader?:boolean;hideNav?:boolean}){
 ...
 return <><main className={`shell ${hideHeader?"shell-no-header":""} ${hideNav?"shell-no-nav":""}`}>{!hideHeader&&(inner?<header className="inner-topbar"><button onClick={goBack} className="inner-back" aria-label="Voltar"><ChevronLeft size={22}/></button><strong>{label}</strong><Link href={user?"/perfil":"/entrar"} className="inner-avatar" aria-label={user?"Abrir perfil":"Entrar"}>{initials}</Link></header>:<header className="topbar"><Link href="/" className="brand"><span className="brand-cross">✣</span><span><b>Vinde</b><small>Fé que acompanha você.</small></span></Link><Link href={user?"/perfil":"/entrar"} className="avatar" aria-label={user?"Abrir perfil":"Entrar"}>{initials}</Link></header>)}{children}</main>{!hideNav&&<nav className="bottomnav">{links.map(([href,label,Icon])=>{const active=href==="/"?path==="/":path.startsWith(href);return <Link onClick={()=>track("nav_click",{entity_type:"navigation",path:href,parish_id:profile?.primary_parish_id||undefined,metadata:{label}})} className={`navitem ${active?"active":""}`} href={href} key={href}><Icon size={20}/><span>{label}</span></Link>})}</nav>}</>;
}
