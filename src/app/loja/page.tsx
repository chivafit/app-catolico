...
import {ShoppingCart,Search,Plus,BookOpen,Heart,ArrowRight,Gift} from "lucide-react";
...
export default function Loja(){
 ...
 return <AppShell><div className={shop.page}>
  ...
  <section className={shop.editorial}><button className={shop.editorialCard} onClick={()=>setCat(catBy("livr"))}><span><BookOpen size={17}/></span><div><strong>Para ler</strong><small>Livros para aprofundar a caminhada.</small></div></button><button className={shop.editorialCard} onClick={()=>setCat(catBy("present"))}><span><Gift size={17}/></span><div><strong>Para presentear</strong><small>Gestos simples, cheios de significado.</small></div></button></section>
