...
export default function Peregrinacoes(){
 ...
 const upcoming=list.filter(t=>t.id!==hero?.id).slice(0,5);
 return <AppShell><div className={styles.page}>
  ...
  <section className={styles.section}><div className={styles.sectionHead}><h2>Próximas viagens</h2><span>{upcoming.length} {upcoming.length===1?"opção":"opções"}</span></div>{upcoming.length?<div className={styles.pilgRows}>{upcoming.map(t=>{const d=Array.isArray(t.pilgrimage_destinations)?t.pilgrimage_destinations[0]:t.pilgrimage_destinations;return <Link key={t.id} href={`/peregrinacoes/${d?.slug||"aparecida"}?trip=${t.id}`} className={styles.pilgRow}><img src={t.image_url||d?.hero_image_url||fallback} alt=""/><div><strong>{t.title||t.destination}</strong><small>{new Date(t.starts_at).toLocaleDateString("pt-BR")} · {brl(t.price_cents)}</small>{t.origin_city&&<small>Saída de {t.origin_city}</small>}</div><ChevronRight size={18}/></Link>})}</div>:<div className={styles.empty}>A viagem em destaque é a única disponível no momento. Novas saídas aparecerão aqui assim que forem publicadas.</div>}</section>
