...
import {Search,ChevronRight,ChevronDown,CalendarDays,MapPin,Church,Route,HeartHandshake} from "lucide-react";
import styles from "@/app/vindeReference.module.css";
import pilg from "./peregrinacoes.module.css";
...
export default function Peregrinacoes(){
 ...
 return <AppShell><div className={styles.page}>
  ...
  <div className={styles.filterRow}>
   <label className={pilg.filterControl}><span>{origin||"Todas as saídas"}</span><ChevronDown size={15}/><select aria-label="Filtrar por cidade de saída" value={origin} onChange={e=>setOrigin(e.target.value)}><option value="">Todas as saídas</option>{origins.map((x:any)=><option key={x}>{x}</option>)}</select></label>
   <label className={pilg.filterControl}><span>{period||"Qualquer período"}</span><ChevronDown size={15}/><select aria-label="Filtrar por período" value={period} onChange={e=>setPeriod(e.target.value)}><option value="">Qualquer período</option><option value="Próximas viagens">Próximas viagens</option></select></label>
  </div>
  ...
  {!loading&&hero&&<Link href={`/peregrinacoes/${heroDest?.slug||"aparecida"}?trip=${hero.id}`} className={styles.pilgHero} style={{backgroundImage:`url(${hero.image_url||heroDest?.hero_image_url||fallback})`}}>
   <div className={pilg.heroBadges}><span>EM DESTAQUE</span>{heroSpots!=null&&heroSpots>0&&<span className={pilg.availability}>{heroSpots} {heroSpots===1?"vaga restante":"vagas restantes"}</span>}</div>
   <h2>{hero.title||hero.destination}</h2>
   <p>{hero.destination||heroDest?.name}</p>
   <div className={pilg.heroMeta}><span><CalendarDays size={13}/>{dateLabel(hero.starts_at)}</span>{hero.origin_city&&<span>Saída de {hero.origin_city}</span>}</div>
