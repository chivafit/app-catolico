...
import {Search,MapPin,Clock3,ChevronRight,Navigation,Church,CalendarDays,Bell,Heart,UsersRound} from "lucide-react";
...
export default function Igreja(){
 ...
 return <AppShell><div className={`${styles.page} ${styles.communityV2}`}>
  <div className={styles.communityHero} style={{backgroundImage:`linear-gradient(0deg,rgba(16,35,58,.84),rgba(16,35,58,.08)),url(${selected?.photo_url||fallback})`}}><span>COMUNIDADE VINDE</span><h1>{selected?.name||"Sua comunidade"}</h1><p>{selected?.city||"Piumhi"} · fé vivida perto de você</p></div>
  ...
  <div className={styles.communityActions}><a href={directions} target="_blank" rel="noreferrer"><Navigation/><span>Como chegar</span></a><a href="#horarios"><Clock3/><span>Horários</span></a><a href="#eventos"><CalendarDays/><span>Eventos</span></a><Link href="/campanhas"><Heart/><span>Doações</span></Link></div>
  ...
  {selected&&<section className={styles.communityCard}><div className={styles.communityCardTop}><img src={selected.photo_url||fallback} alt=""/><div><small>SUA COMUNIDADE</small><h2>{selected.name}</h2><p><MapPin size={12}/>{selected.address||`${selected.city||""} - ${selected.state||""}`}</p></div></div><div className={styles.communityMass} id="horarios"><Clock3 size={18}/><div><small>MISSAS HOJE</small><strong>{todayMasses.length?todayMasses.map(m=>hh(m.starts_at)).join(" · "):"Consulte os horários"}</strong></div></div></section>}
  {parishPosts.length>0&&<section className={styles.section}><div className={styles.sectionHead}><h2>Hoje na comunidade</h2><span>Avisos</span></div><div className={styles.communityNotices}>{parishPosts.map(p=><div key={p.id}><Bell size={17}/><div><strong>{p.title}</strong><p>{p.body}</p></div></div>)}</div></section>}
  <section className={styles.section} id="eventos"><div className={styles.sectionHead}><h2>Próximos encontros</h2><Link href="/eventos">Ver todos</Link></div>{parishEvents.length?<div className={styles.communityEvents}>{parishEvents.map(e=><Link href="/eventos" key={e.id}><span><CalendarDays size={16}/></span><div><strong>{e.title}</strong><small>{new Date(e.starts_at).toLocaleDateString("pt-BR",{day:"2-digit",month:"long"})}{e.location?` · ${e.location}`:""}</small></div><ChevronRight size={17}/></Link>)}</div>:<div className={styles.empty}>Nenhum evento publicado para esta comunidade.</div>}</section>
  <section className={styles.section}><div className={styles.sectionHead}><h2>Outras igrejas</h2><span>{filtered.length}</span></div><div className={styles.communityChurchList}>{filtered.map(p=><button type="button" key={p.id} className={p.id===selected?.id?styles.selected:""} onClick={()=>setSelectedId(p.id)}><img src={p.photo_url||fallback} alt=""/><div><strong>{p.name}</strong><small><MapPin size={10}/>{p.address||p.city||"Piumhi"}</small></div><ChevronRight size={17}/></button>)}</div></section>
  <Link href="/explorar" className={styles.communityClosing}><UsersRound size={20}/><div><small>VIVA MAIS PERTO</small><strong>Explore tudo o que acontece no Vinde.</strong></div><ChevronRight size={17}/></Link>
 </div></AppShell>
}
