...
create table public.daily_devotionals (
  id uuid primary key default gen_random_uuid(),
  devotional_date date unique not null,
  title text not null,
...
create table public.orders (
  ...
  parish_share_cents integer not null default 0,
...
create table public.pilgrimages (
  id uuid primary key default gen_random_uuid(),
  organizer_id uuid references public.travel_organizers(id),
  title text not null,
  origin_city text,
...
create table public.transactions (
  ...
  parish_share_cents integer default 0,
  partner_share_cents integer default 0,
  status text default 'pending',
...
create policy "public parishes readable" on public.parishes for select using (true);
create policy "published devotionals readable" on public.daily_devotionals for select using (published = true);
create policy "public events readable" on public.events for select using (status = 'published');
create policy "public campaigns readable" on public.campaigns for select using (status = 'published');
create policy "active products readable" on public.products for select using (active = true);
create policy "published pilgrimages readable" on public.pilgrimages for select using (status = 'published');
create policy "own profile readable" on public.profiles for select using (auth.uid() = id);
create policy "own profile update" on public.profiles for update using (auth.uid() = id);
create policy "own completions" on public.devotional_completions for all using (auth.uid() = user_id) with check (auth.uid() = user_id);
create policy "own orders readable" on public.orders for select using (auth.uid() = user_id);
create policy "own bookings readable" on public.bookings for select using (auth.uid() = user_id);
create policy "own transactions readable" on public.transactions for select using (auth.uid() = user_id);
